...
**2. 시그널이 나온 날의 종가로 바로 체결됩니다** `백테스트 편향` ✅ 해결

당일 종가로 이동평균과 교차 여부를 계산한 뒤, 같은 종가에 매수·매도한 것으로 처리했습니다. 실제로는 장이 끝나야 시그널을 알 수 있으므로, 거래 시점에 아직 알 수 없는 정보를 쓴 셈(look-ahead bias)이라 실제로 재현할 수 없는 결과가 나옵니다.

시그널을 하루 늦춰 **다음 거래일 종가에 체결**하도록 고쳤습니다. 모든 주문이 같은 방향 시그널의 다음 거래일에 체결되는 것을 확인했습니다. 화면의 "최근 시그널"은 시그널이 발생한 날짜를 그대로 보여줍니다.
